...
<script lang="ts">
  ...
  export default class ListTeacherClarificationQuestionsView extends Vue {
    ...
    async showResponse(clarificationQuestion: ClarificationQuestion){
      ...
      if(this.currentClarificationQuestion.id != null)
        await this.$router.push({ name: 'show-teacher-clarification-responses' , params: {clarificationQuestionId : this.currentClarificationQuestion.id.toString()},});
    }
  ...
  async onSaveClarificationResponse(clarificationResponse: ClarificationResponse) {
    this.clarificationResponses = this.clarificationResponses.filter(cR => cR.id !== clarificationResponse.id);
