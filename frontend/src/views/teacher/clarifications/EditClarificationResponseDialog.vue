        ...
        <span class="headline">
          Answer Clarification
            ...
            <v-card-actions>
                ...
                >Cancel</v-btn
                >
                <v-btn color="blue darken-1" @click="saveClarificationResponse" data-cy="saveButton"
                >Save</v-btn
                >
            </v-card-actions>
        </v-card>
...
<script lang="ts">
  ...
  export default class EditClarificationResponseDialog extends Vue {
    ...
    @Prop({ type: ClarificationQuestion, required: true }) readonly clarificationQuestion!: ClarificationQuestion;
    @Prop({ type: ClarificationResponse, required: true }) readonly clarificationResponse!: ClarificationResponse;

    editClarificationResponse!: ClarificationResponse;

    created() {
      this.editClarificationResponse = new ClarificationResponse(this.clarificationResponse);
    }
    ...
    async saveClarificationResponse() {
      ...
      if (this.editClarificationResponse) {
        try {
          const result = await RemoteServices.createClarificationResponse(this.clarificationQuestion.id,this.editClarificationResponse);
